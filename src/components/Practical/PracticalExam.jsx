...
import PracticalRecord from "./PracticalRecord";
...
import practicalExamFetch from "../../actions/PracticalViva/practicalExamFetch";
import practicalQuestionIndex from "../../actions/PracticalViva/practicalQuestionIndex";
import practicalUploadAnswer from "../../actions/PracticalViva/practicalUploadAnswer";
import practicalExamSubmit from "../../actions/PracticalViva/practicalExamSubmit";

const PracticalExam = () => {
  const [practicalExamQuestion, setPracticalExamQuestion] = useState([]);
  ...
  const getPracticalExamQuestion = async () => {
    try {
      ...
      console.log(data);
      const response = await practicalExamFetch(data);
      console.log(response);
      if (response.data.code === 1000) {
        setPracticalExamQuestion(response.data.exams);
      }
    } catch (error) {
  ...
  const handleQuestionIndicatorClick = async (index) => {
    if (marks) {
      await practicalAnswerUploadHandler();
      present;
    }
    setCurrentIndex(index);
    await handlePracticalQuestionByIndex(index);
  ...
  const handleNextQuestion = async () => {
    ...
    if (currentIndex === practicalExamQuestion.totalq) {
      await handlePracticalExamSubmit();
      swal({
        title: "Thanks",
        icon: "success",
      });
    ...
    if (currentIndex <= practicalExamQuestion.totalq) {
      ...
      await handlePracticalQuestionByIndex(nextIndex);
  ...
  const handlePreviousQuestion = async () => {
    if (currentIndex > 1) {
      ...
      await handlePracticalQuestionByIndex(previousIndex);
  ...
  const handlePracticalQuestionByIndex = async (index) => {
    try {
      const user = JSON.parse(localStorage.getItem("assessor_user"));
      const data = {
        usercode: user?.usercode,
      ...
      console.log(data);
      const response = await practicalQuestionIndex(data);
      if (response.data.code === 1000) {
        // Update state with the new question
        setPracticalExamQuestion((prevQuestions) => ({
          ...prevQuestions,
  ...
  const practicalAnswerUploadHandler = async () => {
    try {
      const user = JSON.parse(localStorage.getItem("assessor_user"));
      console.log("user :", user);
      const maxMarks = practicalExamQuestion?.question?.max_marks;
      if (marks > maxMarks) {
        swal({
          icon: "error",
          title: "Invalid Marks",
      ...
      const question_id = practicalExamQuestion?.question?.question_id;
      ...
      console.log(data);
      const response = await practicalUploadAnswer(data);
      console.log(response);
  ...
  const renderQuestionIndicators = () => {
    const indicators = [];
    for (let i = 0; i < practicalExamQuestion.totalq; i++) {
      let bgColor = "#ffffff";
  ...
  const handlePracticalExamSubmit = async () => {
    try {
      const user = JSON.parse(localStorage.getItem("assessor_user"));
      const data = {
        usercode: user?.usercode,
      ...
      console.log(data);
      const response = await practicalExamSubmit(data);
      if (response.data.code === 1000) {
      }
    } catch (error) {
      console.log("Error while getting data :: ", error);
            ...
            <div className="bg-[#DDEAFF] rounded-lg w-full">
              <div className="py-3 flex justify-between w-full px-8">
                <div className="flex items-center ">
                  <span className="text-[#1C4481] font-semibold">
                    {currentIndex + 1}/{practicalExamQuestion.totalq}- Level (MT
                    Nos-
                    {practicalExamQuestion?.question?.diff_level})
                  </span>
                </div>
                <div className="flex gap-4">
                  <div className="flex border-[1px] border-blue-600 py-2 px-8 bg-white rounded">
                    <span className="font-bold text-blue-600 text-nowrap">
                      {" "}
                      Max Marks-{practicalExamQuestion?.question?.max_marks}
                    </span>
                  </div>
                  <div className="flex justify-center items-center bg-yellow-300 rounded-full w-auto px-4">
                    <span className="font-semibold text-nowrap">
                      {practicalExamQuestion?.question?.question_inst}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex px-8 justify-between flex-col py-4">
                <span className="text-black font-bold w-full ">
                  Q- {practicalExamQuestion?.question?.question}
                </span>
                <PracticalRecord
            ...
            <div className=" mx-4 rounded-xl p-6 flex flex-col gap-4 justify-center items-center">
              ...
              <div className="border rounded-full">
                <input
                  type="number"
                  name="mark"
                  placeholder={`0-${practicalExamQuestion?.question?.max_marks}`}
