...
import pcVivaStudentList from "../../actions/PC-Viva/pcVivaStudentList";

function PCVivaStudentList() {
  const [pvVivaStudentLists, setPCVivaStudentLists] = useState([]);
  ...
  const getPCVivaStudentList = async () => {
    try {
      ...
      console.log(data);
      const response = await pcVivaStudentList(data);
      console.log(" PC Viva Student list", response);
      if (response?.data?.code === 1000)
        setPCVivaStudentLists(response?.data?.students);
      setLoading(false);
    ...
    <div className="flex flex-col">
      ...
      <div className="min-h-screen p-4 bg-[#EDEDED] flex">
        <div className="bg-white p-4 w-full rounded-2xl">
          ...
          {pvVivaStudentLists.length > 0 &&
            activeExam_pc.viva_pc === 1 &&
            pvVivaStudentLists.map((data) => (
              <>
